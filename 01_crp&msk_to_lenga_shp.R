...
files <- list.files("~/PROJECTS/16b_trapananda_TS/00_MOD13_QA_testt/00_rawdata", pattern = glob2rx("*.tif"), full.names =T) 
r1 <- raster(files[1])

lenga <- readOGR(dsn = path.expand("~/PROJECTS/16b_trapananda_TS/shp"), layer = "Lenga_distribution_UTMWGS8419S")
cuenca <- readOGR(dsn = path.expand("~/PROJECTS/16b_trapananda_TS/shp"), layer = "subcuenca_manihuales")
...
dir <- "~/PROJECTS/16b_trapananda_TS/00_MOD13_QA_testt/02_msk_lenga"
